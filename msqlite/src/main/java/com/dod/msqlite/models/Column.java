package com.dod.msqlite.models;

import android.database.Cursor;
import android.util.Log;

import com.dod.msqlite.Annotations;
import com.dod.msqlite.DataTypes;
import com.dod.msqlite.SerializationUtils;
...
{	
	...
	public Column(Field field, Table table)
	{
		...
		if (field.isAnnotationPresent(Annotations.ColumnName.class)) this.name = field.getAnnotation(Annotations.ColumnName.class).value();
		else this.name = field.getName();
		
		if (field.isAnnotationPresent(Annotations.Default.class)) this.defultValue = field.getAnnotation(Annotations.Default.class).value();
		...
		if (field.isAnnotationPresent(Annotations.DataType.class))
			this.dataType = field.getAnnotation(Annotations.DataType.class).value();
		else 
